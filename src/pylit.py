...
# ---------------
# 
# Collect option defaults in a dictionary. This facilitates
...
# command line option). Setting it in `option_defaults` would override this
# auto-setting feature::
...
# `option_defaults`_ dictionary and can be overridden by matching keyword
...
# on command line options and sensible defaults::
    ...
    def __init__(self, args=sys.argv[1:], **keyw):
        ...
        p.add_option("--language", action="store", 
                     choices = option_defaults["languages"].values(),
...
# ::
    ...
    def complete_values(self, values):
        ...
        if values.txt2code is None:
            ...
            values.txt2code = not (out_extension 
                                   in self.values.text_extensions)
        ...
        values.ensure_value("language", 
                            self.values.languages.get(code_extension, "python"))
